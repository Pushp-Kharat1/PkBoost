...
fn subtract_hists(
    ...
    parent_hists.iter().zip(sibling_hists.iter())
...
fn find_best_split_across_features(
    ...
    hists.iter().enumerate()
        .map(|(feat_idx_local, hist)| (feat_idx_local, find_best_split_cached(hist, params, depth)))
        .reduce(|a, b| if a.1.best_gain > b.1.best_gain { a } else { b })
}
