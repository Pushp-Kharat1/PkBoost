...
impl MSELoss {
    ...
    pub fn gradient(&self, y_true: &[f64], y_pred: &[f64]) -> Vec<f64> {
        par_zip_map(y_pred, y_true, |&pred, &true_y| pred - true_y)
    }
...
impl PKBoostRegressor {
    ...
    pub fn fit(&mut self, x: &Vec<Vec<f64>>, y: &[f64], eval_set: Option<(&Vec<Vec<f64>>, &[f64])>, verbose: bool) -> Result<(), String> {
        ...
        for iter in 0..self.n_estimators {
            ...
            let tree_preds: Vec<f64> = par_map_range(0..n_samples, |i| {
                tree.predict_from_transposed(&transposed, i)
            });
            
            for (p, &tp) in train_preds.iter_mut().zip(tree_preds.iter()) {
                *p += self.learning_rate * tp;
            }
            
            if let (Some(ref vt), Some(ref mut vp), Some((_, yv))) = (val_trans.as_ref(), val_preds.as_mut(), eval_set) {
                let val_tree_preds: Vec<f64> = par_map_range(0..vt.n_samples, |i| {
                    tree.predict_from_transposed(vt, i)
                });
                
                for (p, &tp) in vp.iter_mut().zip(val_tree_preds.iter()) {
                    *p += self.learning_rate * tp;
                }
