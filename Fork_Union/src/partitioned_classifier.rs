...
impl PartitionedClassifier {
    ...
    pub fn train_specialists_with_validation(
        ...
        let specialists: std::sync::Arc<Vec<fork_union::SpinMutex<Result<OptimizedPKBoostShannon, String>>>> = 
            std::sync::Arc::new((0..self.config.n_partitions).map(|_| fork_union::SpinMutex::new(Err(String::new()))).collect());
        let specialists_clone = std::sync::Arc::clone(&specialists);
        
        (0..self.config.n_partitions).into_par_iter().with_pool(&mut pool).for_each(|partition_id| {
            let result = (|| {
            ...
            *specialists_clone[partition_id].lock() = result;
        });
        
        let specialists: Vec<_> = std::sync::Arc::try_unwrap(specialists).ok().unwrap()
            .into_iter().map(|m| m.into_inner()).collect::<Result<Vec<_>, String>>()?;
