...
class PKBoostClassifier(ClassifierMixin, BaseEstimator):
    ...
    def predict_proba(self, X):
        ...
        if X.shape[1] != self.n_features_in_:
            raise ValueError(
                f"X has {X.shape[1]} features, but PKBoostClassifier was fitted with {self.n_features_in_} features."
            )
...
class PKBoostRegressor(RegressorMixin, BaseEstimator):
    ...
    def predict(self, X):
        ...
        if X.shape[1] != self.n_features_in_:
            raise ValueError(f"X has {X.shape[1]} features, but was fitted with {self.n_features_in_} features.")
