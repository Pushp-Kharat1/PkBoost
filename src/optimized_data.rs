...
impl TransposedData {
    ...
    pub fn build_multi_histogram_4x(
        ...
        {
            for i in 0..len {
                let s = active.samples[i];
                let b0 = pre_bins[i * 4] as usize;
                let b1 = pre_bins[i * 4 + 1] as usize;
                let b2 = pre_bins[i * 4 + 2] as usize;
                let b3 = pre_bins[i * 4 + 3] as usize;
                outputs[0].bins[b0].grad += s.grad;
                outputs[0].bins[b0].hess += s.hess;
                outputs[0].bins[b0].y += s.y;
                outputs[0].bins[b0].count += s.count;
                outputs[1].bins[b1].grad += s.grad;
                outputs[1].bins[b1].hess += s.hess;
                outputs[1].bins[b1].y += s.y;
                outputs[1].bins[b1].count += s.count;
                outputs[2].bins[b2].grad += s.grad;
                outputs[2].bins[b2].hess += s.hess;
                outputs[2].bins[b2].y += s.y;
                outputs[2].bins[b2].count += s.count;
                outputs[3].bins[b3].grad += s.grad;
                outputs[3].bins[b3].hess += s.hess;
                outputs[3].bins[b3].y += s.y;
                outputs[3].bins[b3].count += s.count;
            }
        }
...
impl CachedHistogram {
    ...
    pub fn subtract_into(&self, other: &Self, out: &mut Self) {
        ...
        #[cfg(not(any(target_arch = "x86", target_arch = "x86_64")))]
        for i in 0..n {
            out.bins[i].grad = self.bins[i].grad - other.bins[i].grad;
            out.bins[i].hess = self.bins[i].hess - other.bins[i].hess;
            out.bins[i].y = self.bins[i].y - other.bins[i].y;
            out.bins[i].count = self.bins[i].count - other.bins[i].count;
        }
